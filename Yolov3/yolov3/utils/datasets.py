...
class LoadTensor:
    def __init__(self, tensor, half=False):
    ...
    def __next__(self):
        ...
        # Read image from tensor and convert to numpy array (img0)
        img0 = (self.tensor[0, self.count].cpu().numpy() +1)* 127.5  # Convert 0-1 to 0-255
        img0 = np.uint8(np.expand_dims(img0, axis=0))  # Add channel dimension
        img0 = np.tile(img0, (3, 1, 1))  # Convert single channel to three channels (RGB)
        img0 = np.transpose(img0, (1, 2, 0))  # Change channel position
