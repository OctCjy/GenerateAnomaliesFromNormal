...
def create_pseudoanomaly_cifar_smooth(img, cifar_img, max_size, h, w, dataset, max_move=0,return_ratio=False):
    assert 0 <= max_size <= 1
    ...
    rand_width = np.random.uniform(high=max_size)
    rand_height = np.random.uniform(high=max_size) 
    x_sigma = max(10,int(rand_width*w))
    y_sigma = max(10,int(rand_height*h))
    if max_move == 0:
    ...
    if return_ratio:
        return img, mask,rand_width*rand_height*20
    else:
        return img, mask
...
def create_pseudoanomaly_dark_smoothborder(img, max_size, h, w, max_move=0,num=10):
    
    assert 0 <= max_size <= 1

    for _ in range(num):
        # Generate a random color for the pure color image
        color = np.random.uniform(0, 1)

        cifar_img = torch.ones((1, h, w)) * color
